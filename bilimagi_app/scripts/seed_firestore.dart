...
/// Seed script for Bilimagi MVP demo data
...
void main() async {
  ...
  try {
    // Step 1: Create demo users in Firestore (Auth'ta zaten var)
    print('📝 Creating user profiles...');
    await _seedUsers(firestore);

    // Step 2: Create communities
    print('🏘️  Creating communities...');
    final communityIds = await _seedCommunities(firestore);

    // Step 3: Create weeks for each community
    print('📅 Creating weeks...');
    final weekIds = await _seedWeeks(firestore, communityIds);

    // Step 4: Create articles for each week
    print('📄 Creating articles...');
    await _seedArticles(firestore, weekIds);
...
Future<void> _seedUsers(FirebaseFirestore firestore) async {
  // Demo kullanıcıları (Auth'ta email/password ile oluşturulmuş olmalı)
  // Burada sadece Firestore user profilleri oluşturuyoruz
  final users = [
    {
      'uid': '20ShZTwj7eWHvkH07yJfS3Dmbim2',
      'role': 'admin',
      'displayName': 'Admin User',
      'communityIds': [], // Tüm topluluklara erişebilir
    },
    {
      'uid': 'MT0fNp0jykPo4ecCLGRQi5tBOZi1',
      'role': 'member',
      'displayName': 'Ahmet Yılmaz',
      'communityIds': ['physics-community', 'biology-community'],
    },
    {
      'uid': 'TeLiTsUfZ1cjvYLSAhdFCi8nLqa2',
      'role': 'member',
...
Future<List<String>> _seedCommunities(FirebaseFirestore firestore) async {
  final communities = [
    {
      ...
      'ownerUid': '20ShZTwj7eWHvkH07yJfS3Dmbim2', // Admin UID
      'currentWeekId': null, // İlk week oluşturulduktan sonra güncellenecek
    },
    {
  ...
  for (final community in communities) {
    ...
    await firestore.collection('communities').doc(id).set({
      'name': community['name'],
...
Future<List<Map<String, String>>> _seedWeeks(
  ...
  for (final communityId in communityIds) {
    ...
    await firestore.collection('weeks').doc(weekId).set({
      'communityId': communityId,
      'phase': 'voting', // Başlangıçta voting fazında
      'createdAt': FieldValue.serverTimestamp(),
    });

    // Community'nin currentWeekId'sini güncelle
...
Future<void> _seedArticles(
  FirebaseFirestore firestore,
  List<Map<String, String>> weekIds,
) async {
  // Physics community articles
  ...
  for (int i = 0; i < physicsArticles.length; i++) {
    ...
    print('  ✓ Article: ${physicsArticles[i]['title']?.substring(0, 40)}...');
  }

  // Biology community articles
  ...
  for (int i = 0; i < biologyArticles.length; i++) {
    ...
    print('  ✓ Article: ${biologyArticles[i]['title']?.substring(0, 40)}...');
  }
}
